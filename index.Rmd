...
```{r}
unique(board_games$category)
```

The board games data used in this report is obtained from the [Board Game Geek](https://boardgamegeek.com/) database. The dataset consists of `r ncol(board_games)` columns and `r nrow(board_games)` rows, in which there are `r ncol(board_games)` variables and `r nrow(board_games)` observations, that contains multiple columns consist of data such as max/min playtime, max/min players, min age of players that can play, game designer, game publisher, mechanics of the game and a lot more.The variables included in the data are as follows:
