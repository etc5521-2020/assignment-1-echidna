...
**4. How have the current best categories evolved over time, with respect to average rating?**
...
categ <- categ %>% 
  separate(col = category, into = c("categ1", "categ2", "categ3"), sep = "\\,")
...
categ_iso <- categ %>%
  ...
  group_by(categ1) %>% 
  mutate(mean_rating = mean(average_rating))

topcat <- data.frame(unique(categ_iso$categ1))

topcat <- topcat %>% 
  rename(categ1 = unique.categ_iso.categ1.)

categ_iso1 <- categ_iso %>% 
  select(categ1, mean_rating)

topcat <- topcat %>% 
  left_join(categ_iso1, by = "categ1")

topcat <- distinct(topcat)
...
catp1 <- topcat %>% 
  ggplot(aes(reorder(categ1, -mean_rating), mean_rating, fill = categ1, label = categ1)) +
  geom_col() +
  geom_text(size = 2.5, angle = 90, position = position_stack(vjust = 1.09)) +
  theme(axis.text.x = element_blank(),
        legend.position = "none") +
  xlab("Board game category") +
  ylab("Average rating") +
...
* An interesting point to note about human behavior and how we like to pass our free time, when we see that  **6 out of those top 10 categories of board games are war related.** 
