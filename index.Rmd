...
3. Which game designer was most successful in producing popular games?
...
* Initial Data Analysis is a process which helps one get a feel of the data in question. This helps us have
an overview of the data and gives insights about potential Exlporatory Data Analyis (EDA).
* Initial data analysis is the process of data inspection steps to be carried out after the research plan and
data collection have been finished but before formal statistical analyses. The purpose is to minimize
the risk of incorrect or misleading results. Link for more info
* IDA can be divided into 3 main steps:
  + Data cleaning is the identification of inconsistencies in the data and the resolution of any such
issues.
...
```{r visdatData, fig.cap='Visualization of Data Types', fig.align = "center", fig.height=3, fig.width=6}
vis_dat(board_games)
```

```{r vismissData, fig.cap='Visualization of Missing Values', fig.align = "center", fig.height=3, fig.width=6}
vis_miss(board_games)
```
...
```{r visdatTop50, fig.cap='Visualization of Data Types in Top 50 Games', fig.align = "center", fig.height=3, fig.width=6}
vis_dat(board_games_top50)
```

```{r vismissTop50, fig.cap='Visualization of Missing Values in Top 50 Games', fig.align = "center", fig.height=3, fig.width=6}
vis_miss(board_games_top50)
```
...
bpmaxplaytime <- board_games_top50 %>%
  dplyr::filter(max_playtime < 738.75) %>% ggplot() +
  geom_boxplot(mapping = aes(y= max_playtime)) +
  geom_hline(yintercept = median(board_games_top50$max_playtime), color='blue')
```

```{r MaxPlaytimePlot, fig.align = "center", fig.height=3, fig.width=6}
...
bpminplaytime <- board_games_top50 %>% dplyr::filter(min_playtime < 382.5) %>% ggplot() +
  geom_boxplot(mapping = aes(y= min_playtime)) +
  geom_hline(yintercept = median(board_games_top50$min_playtime), color='blue')
```

```{r MinPlaytimePlot, fig.align = "center", fig.height=3, fig.width=6}
p2 <- board_games_top50 %>%
  dplyr::filter(min_playtime < 382.5) %>%
  ggplot(aes(min_playtime, average_rating, colour = name)) +
  geom_point(size = 2) +
  theme(legend.position = "none") +
  geom_vline(xintercept = median(board_games_top50$min_playtime), color='blue') +
...
bpminplayers <- board_games_top50 %>% ggplot() +
  geom_boxplot(mapping = aes(y= min_players)) +
  geom_hline(yintercept = median(board_games_top50$min_players), color='green')
```

```{r MinPlayersPlot, fig.align = "center", fig.height=3, fig.width=6}
p3 <- board_games_top50 %>%
  ggplot(aes(min_players, average_rating, colour = name)) +
  geom_point(size = 2) +
  theme(legend.position = "none") +
  geom_vline(xintercept = median(board_games_top50$min_players), color='green') +
...
bpmaxplayers <- board_games_top50 %>% ggplot() +
  geom_boxplot(mapping = aes(y= max_players)) +
  geom_hline(yintercept = median(board_games_top50$max_players), color='green')
```

```{r MaxPlayersPlot, fig.align = "center", fig.height=3, fig.width=6}
p4 <- board_games_top50 %>%
  dplyr::filter(max_players <15) %>%
  ggplot(aes(max_players, average_rating, colour = name)) +
  geom_point(size = 2) +
  theme(legend.position = "none") +
  geom_vline(xintercept = median(board_games_top50$max_players), color='green')+
...
bpminage <- board_games_top50 %>% ggplot() +
  geom_boxplot(mapping = aes(y= min_age)) +
  geom_hline(yintercept = median(board_games_top50$min_age), color='red')
```

```{r MinAgePlot, fig.align = "center", fig.height=3, fig.width=6}
...
```{r BoxplotCompare}
gridExtra::grid.arrange(bpmaxplayers, bpminplayers, bpmaxplaytime, bpminplaytime, bpminage)
```
